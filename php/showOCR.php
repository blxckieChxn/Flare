...
<div class="main">  
  ...
  <div class="contenido">
    ...
    <?php
        ...
        function showOCR($conn, $ids){
        
            $query = "SELECT c.idCarta, c.nombre, e.abreviatura, c.precio_eur, c.precio_usd, c.imagen 
                            FROM cartas c JOIN ediciones e ON (c.idEdicion = e.idEdicion)
                            WHERE c.idCarta IN ($ids)";   
        ...
        if($_SESSION["OCR"]){
            $coincidencias = $_SESSION["OCR"];
            $idsSimples = [];
           
            foreach ($coincidencias as $item) {
                $idsSimples = array_merge($idsSimples, $item['ids']);
            }

            // Eliminar duplicados si lo deseas
            $idsSimples = array_unique($idsSimples);
            // Reindexar el array
            $idsSimples = array_values($idsSimples);
            // Se transforma en un string para poder pasarlo como parametro
            // a la consulta SQL en un IN()
            $idsString = implode(', ', $idsSimples);
            //conectar a la BD
            $conn = conectar();
            showOCR($conn, $idsString);
            $conn->close();
