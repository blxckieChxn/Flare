...
<?php
    ...
    function showSales($conn, $abreviatura){
        $queryByEdition = "SELECT c.imagen, c.idCarta, c.precio_eur, c.precio_usd, p.cantidad, v.idVenta 
        FROM cartas c JOIN puesta_en p ON (c.idCarta = p.idCarta) JOIN ediciones e ON (c.idEdicion = e.idEdicion)
        JOIN ventas v ON (p.idVenta = v.idVenta)
        WHERE comprador is null AND e.abreviatura = '$abreviatura'";
        
        $queryAll = "SELECT c.imagen, c.idCarta, c.precio_eur, c.precio_usd, p.cantidad, v.idVenta 
        FROM cartas c JOIN puesta_en p ON (c.idCarta = p.idCarta) JOIN ediciones e ON (c.idEdicion = e.idEdicion)
        JOIN ventas v ON (p.idVenta = v.idVenta)
        ...
        if (mysqli_num_rows($result) > 0){
            ...
            while($row = mysqli_fetch_assoc($result)) {
              ...
              echo $row["precio_eur"] . "€ | " . $row["precio_usd"] . "$";
              echo "<div class='card'>";
              ...
              echo "</div>";
              echo "<div> Cantidad disponible: " . $row["cantidad"] . "</div>";
              echo "<div class='number-input'>";
              // Aquí el name usa el idCarta como índice del array
              echo '<input type="number" name="cantidad[' . $row["idCarta"] . ']" value="0" min="0" max="' . $row["cantidad"] . '" class="cantidad-input" data-precio="' . $row["precio_eur"] . '">';
              echo '<input type="hidden" name="idVenta[' . $row["idCarta"] . ']" value="' . $row["idVenta"] . '">';
              echo "</div>";
              echo "</td>";
              $count = $count + 1;
              $i=$i+1;
          ...
          echo "<h3>Aqui no hay cartas en venta aun</h3>";
        }
        echo "</table>";   
        ...
        echo "<button type='submit' class='botonSave' name='metodo' value=" . "buy" . ">Comprar Cartas</button>";
        echo "</div>";
        echo "</form>";   
    ...
    if($_SESSION["edicion_busqueda_ventas"]){
      // Establecer método (delete)
      $_SESSION["metodo"] = "delete";

      // Recuperar edicion de busqueda
      $abreviatura = $_SESSION["edicion_busqueda_ventas"];
      $conn = conectar();
      showSales($conn, $abreviatura);
      $conn->close();
    }
